...
import 'erply_responsive_framework.dart';
...
class ErplyResponsiveValue<T> {
  ...
  ErplyResponsiveValue(this.context,
      {required this.defaultValue, required this.valueWhen}) {
    // Breakpoint reference check. Verify a parent
    // [ErplyResponsiveWrapper] exists if a reference is found.
    if (valueWhen.firstWhereOrNull((element) => element.name != null) != null) {
      try {
        ErplyResponsiveWrapper.of(context);
      } catch (e) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ErrorSummary(
              'A ErplyResponsiveCondition was caught referencing a nonexistant breakpoint.'),
          ErrorDescription(
              'A ErplyResponsiveCondition requires a parent ErplyResponsiveWrapper '
              'to reference breakpoints. Add a ErplyResponsiveWrapper or remove breakpoint references.')
        ]);
      }
    }
    ...
    List<ErplyResponsiveBreakpointSegment>? segments =
        ErplyResponsiveWrapper.of(context).breakpointSegments;
  ...
  T? getValue(BuildContext context, List<Condition> conditions) {
    ...
    if (ErplyResponsiveWrapper.of(context).orientation ==
  ...
  /// Named breakpoints from a parent [ErplyResponsiveWrapper].
  ...
  Condition? getActiveCondition(
    ...
    Condition? equalsCondition = conditions.firstWhereOrNull((element) {
      if (element.condition == Conditional.EQUALS) {
        return ErplyResponsiveWrapper.of(context).activeBreakpoint.name ==
            element.name;
      }
    ...
    Condition? smallerThanCondition = conditions.firstWhereOrNull((element) {
      if (element.condition == Conditional.SMALLER_THAN) {
        if (element.name != null) {
          return ErplyResponsiveWrapper.of(context)
              .isSmallerThan(element.name!);
        }
    ...
    Condition? largerThanCondition =
      ...
      if (element.condition == Conditional.LARGER_THAN) {
        if (element.name != null) {
          return ErplyResponsiveWrapper.of(context).isLargerThan(element.name);
        }
...
///
/// ErplyResponsiveVisibility accepts [Condition]s in
/// [visibleWhen] and [hiddenWhen] convenience
/// fields. The [child] widget is [visible] by default.
class ErplyResponsiveVisibility extends StatelessWidget {
  final Widget child;
  final bool visible;
  ...
  const ErplyResponsiveVisibility({
    super.key,
    required this.child,
    this.visible = true,
  ...
  Widget build(BuildContext context) {
    ...
    // Get visible value from active condition.
    visibleValue = ErplyResponsiveValue(context,
...
class ErplyResponsiveConstraints extends StatelessWidget {
  final Widget child;
  final BoxConstraints? constraint;
  final List<Condition> constraintsWhen;

  const ErplyResponsiveConstraints(
      {super.key,
      required this.child,
      this.constraint,
  ...
  Widget build(BuildContext context) {
    ...
    constraintValue = ErplyResponsiveValue(context,

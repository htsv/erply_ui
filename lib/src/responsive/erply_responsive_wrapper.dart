...
import 'utils/erply_responsive_utils.dart';
...
/// on [ErplyResponsiveBreakpoint]s. Computed [MediaQueryData]
/// and [ErplyResponsiveWrapperData] with correct
...
/// screen sizes, [ErplyResponsiveWrapper] can proportionally scale
...
/// from the nearest [ErplyResponsiveBreakpoint]. As the
...
class ErplyResponsiveWrapper extends StatefulWidget {
  final Widget? child;
  final List<ErplyResponsiveBreakpoint>? breakpoints;
  ...
  /// active when the [ErplyResponsiveTargetPlatform] is Android, iOS, or Fuchsia.
  /// To enable landscape breakpoints on other platforms, pass a custom
  /// list of supported platforms to [landscapePlatforms].
  final List<ErplyResponsiveBreakpoint>? breakpointsLandscape;
  ...
  final List<ErplyResponsiveTargetPlatform>? landscapePlatforms;
  ...
  /// Control the internal Stack alignment. The ErplyResponsiveWrapper
  ...
  @override
  ErplyResponsiveWrapperState createState() => ErplyResponsiveWrapperState();

  static Widget builder(
    Widget? child, {
    List<ErplyResponsiveBreakpoint>? breakpoints,
    List<ErplyResponsiveBreakpoint>? breakpointsLandscape,
    List<ErplyResponsiveTargetPlatform>? landscapePlatforms,
    double minWidth = 450,
    double? maxWidth,
    String? defaultName,
  ...
  static ErplyResponsiveWrapperData of(BuildContext context) {
    final InheritedErplyResponsiveWrapper? data = context
        .dependOnInheritedWidgetOfExactType<InheritedErplyResponsiveWrapper>();
    if (data != null) return data.data;
    throw FlutterError.fromParts(
      <DiagnosticsNode>[
        ErrorSummary(
            'ErplyResponsiveWrapper.of() called with a context that does not contain a ErplyResponsiveWrapper.'),
        ErrorDescription(
            'No ErplyResponsive ancestor could be found starting from the context that was passed '
            'to ErplyResponsiveWrapper.of(). Place a ErplyResponsiveWrapper at the root of the app '
            'or supply a ErplyResponsiveWrapper.builder.'),
...
class ErplyResponsiveWrapperState extends State<ErplyResponsiveWrapper>
  ...
  List<ErplyResponsiveBreakpoint> breakpoints = [];
  List<ErplyResponsiveBreakpointSegment> breakpointSegments = [];
  ...
  late ErplyResponsiveBreakpointSegment activeBreakpointSegment;
  ...
  static const List<ErplyResponsiveTargetPlatform> _landscapePlatforms = [
    ErplyResponsiveTargetPlatform.iOS,
    ErplyResponsiveTargetPlatform.android,
    ErplyResponsiveTargetPlatform.fuchsia,
  ];

  ErplyResponsiveTargetPlatform? platform;

  void setPlatform() {
    platform = kIsWeb
        ? ErplyResponsiveTargetPlatform.web
  ...
  List<ErplyResponsiveBreakpoint> getActiveBreakpoints() {
  ...
  List<ErplyResponsiveBreakpointSegment> calcBreakpointSegments(
      List<ErplyResponsiveBreakpoint> breakpoints) {
    // Seed breakpoint based on config values.
    ErplyResponsiveBreakpoint defaultBreakpoint = ErplyResponsiveBreakpoint(
        breakpoint: minWidth,
        name: defaultName,
        behavior: defaultScale
            ? ErplyResponsiveBreakpointBehavior.AUTOSCALE
            : ErplyResponsiveBreakpointBehavior.RESIZE,
  ...
  ErplyResponsiveBreakpointSegment getActiveBreakpointSegment(
      double windowWidth) {
    ErplyResponsiveBreakpointSegment activeBreakpointSegment =
        breakpointSegments.reversed
  ...
  void initState() {
    ...
    if (widget.debugLog) {
      List<ErplyResponsiveBreakpoint> defaultBreakpoints =
          widget.breakpoints ?? [];
      List<ErplyResponsiveBreakpointSegment> defaultBreakpointSegments =
      ...
      ErplyResponsiveUtils.debugLogBreakpointSegments(
          defaultBreakpointSegments);
      // Print landscape breakpoints.
      if (widget.breakpointsLandscape != null) {
        List<ErplyResponsiveBreakpoint> landscapeBreakpoints =
            widget.breakpointsLandscape ?? [];
        List<ErplyResponsiveBreakpointSegment> landscapeBreakpointSegments =
  ...
  @override
  void didUpdateWidget(ErplyResponsiveWrapper oldWidget) {
    super.didUpdateWidget(oldWidget);
    // When [ErplyResponsiveWrapper]'s constructor is
  ...
  Widget build(BuildContext context) {
    ...
    return InheritedErplyResponsiveWrapper(
      data: ErplyResponsiveWrapperData.fromErplyResponsiveWrapper(this),
      child: Stack(
...
/// ErplyResponsive data about the current screen.
///
/// Resized and scaled values can be accessed
/// such as [ErplyResponsiveWrapperData.scaledWidth].
@immutable
class ErplyResponsiveWrapperData {
  final double screenWidth;
  final double screenHeight;
  final double scaledWidth;
  final double scaledHeight;
  final List<ErplyResponsiveBreakpoint> breakpoints;
  final List<ErplyResponsiveBreakpointSegment> breakpointSegments;
  final ErplyResponsiveBreakpoint activeBreakpoint;
  final bool isMobile;
  final bool isPhone;
  final bool isTablet;
  ...
  ///
  /// Consider using [ErplyResponsiveWrapperData.fromErplyResponsiveWrapper]
  /// to create data based on the [ErplyResponsiveWrapper] state.
  const ErplyResponsiveWrapperData({
    this.screenWidth = 0,
    this.screenHeight = 0,
    this.scaledWidth = 0,
    this.scaledHeight = 0,
    ...
    this.activeBreakpoint = const ErplyResponsiveBreakpoint.tag(0, name: ''),
    this.isMobile = false,
    this.isPhone = false,
    this.isTablet = false,
  ...
  /// Creates data based on the [ErplyResponsiveWrapper] state.
  static ErplyResponsiveWrapperData fromErplyResponsiveWrapper(
      ErplyResponsiveWrapperState state) {
    return ErplyResponsiveWrapperData(
  ...
  @override
  String toString() =>
      'ErplyResponsiveWrapperData(screenWidth: $screenWidth, screenHeight: $screenHeight, scaledWidth: $scaledWidth, scaledHeight: $scaledHeight, breakpoints: ${breakpoints.asMap()}, breakpointSegments: $breakpointSegments, activeBreakpoint: $activeBreakpoint, isMobile: $isMobile, isPhone: $isPhone, isTablet: $isTablet, isDesktop: $isDesktop)';
  ...
  bool isSmallerThan(String? name) =>
      ...
      breakpointSegments
          .firstWhere((element) => element.responsiveBreakpoint.name == name,
              orElse: () => const ErplyResponsiveBreakpointSegment(
                  breakpoint: 0,
                  segmentType: ErplyResponsiveBreakpointBehavior.TAG,
                  responsiveBreakpoint:
                      ErplyResponsiveBreakpoint.tag(0, name: '')))
          .breakpoint;

  ErplyResponsiveBreakpoint? firstBreakpointNamed(String name) {
    try {
...
/// Creates an immutable widget that exposes [ErplyResponsiveWrapperData]
/// to child widgets.
///
/// Access values such as the [ErplyResponsiveWrapperData.scaledWidth]
/// property through [ErplyResponsiveWrapper.of]
/// `ErplyResponsiveWrapper.of(context).scaledWidth`.
///
/// Querying this widget with [ErplyResponsiveWrapper.of]
/// creates a dependency that causes an automatic
/// rebuild whenever the [ErplyResponsiveWrapperData]
/// changes.
///
/// If no [ErplyResponsiveWrapper] is in scope then the
/// [MediaQuery.of] method will throw an exception,
/// unless the `nullOk` argument is set to true, in
/// which case it returns null.
@immutable
class InheritedErplyResponsiveWrapper extends InheritedWidget {
  final ErplyResponsiveWrapperData data;

  /// Creates a widget that provides [ErplyResponsiveWrapperData] to its descendants.
  ///
  /// The [data] and [child] arguments must not be null.
  const InheritedErplyResponsiveWrapper(
      {super.key, required this.data, required super.child});

  @override
  bool updateShouldNotify(InheritedErplyResponsiveWrapper oldWidget) =>
      data != oldWidget.data;
}

enum ErplyResponsiveBreakpointBehavior {
  RESIZE,
  AUTOSCALE,
  AUTOSCALEDOWN,
  TAG,
...
/// breakpoints.
@immutable
class ErplyResponsiveBreakpoint {
  final double breakpoint;
  final String? name;
  final ErplyResponsiveBreakpointBehavior behavior;
  final double scaleFactor;

  @visibleForTesting
  const ErplyResponsiveBreakpoint(
      {required this.breakpoint,
      this.name,
      this.behavior = ErplyResponsiveBreakpointBehavior.RESIZE,
      this.scaleFactor = 1});

  const ErplyResponsiveBreakpoint.resize(this.breakpoint,
      {this.name, this.scaleFactor = 1})
      : behavior = ErplyResponsiveBreakpointBehavior.RESIZE,
        assert(breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ErplyResponsiveWrapper`.');

  const ErplyResponsiveBreakpoint.autoScale(this.breakpoint,
      {this.name, this.scaleFactor = 1})
      : behavior = ErplyResponsiveBreakpointBehavior.AUTOSCALE,
        assert(breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ErplyResponsiveWrapper`.');

  const ErplyResponsiveBreakpoint.autoScaleDown(this.breakpoint,
      {this.name, this.scaleFactor = 1})
      : behavior = ErplyResponsiveBreakpointBehavior.AUTOSCALEDOWN,
        assert(breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ErplyResponsiveWrapper`.');

  const ErplyResponsiveBreakpoint.tag(this.breakpoint,
      {required String this.name})
      : behavior = ErplyResponsiveBreakpointBehavior.TAG,
        scaleFactor = 1,
        assert(breakpoint >= 0,
            'Breakpoints cannot be negative. To control behavior from 0, set `default` parameters in `ErplyResponsiveWrapper`.');

  get isResize => behavior == ErplyResponsiveBreakpointBehavior.RESIZE;

  get isAutoScale => behavior == ErplyResponsiveBreakpointBehavior.AUTOSCALE;

  get isAutoScaleDown =>
      behavior == ErplyResponsiveBreakpointBehavior.AUTOSCALEDOWN;

  get isTag => behavior == ErplyResponsiveBreakpointBehavior.TAG;

  ErplyResponsiveBreakpoint copyWith({
    double? breakpoint,
    String? name,
    ErplyResponsiveBreakpointBehavior? behavior,
    double? scaleFactor,
  }) =>
      ErplyResponsiveBreakpoint(
  ...
  /// [responsiveBreakpoint].
  ErplyResponsiveBreakpoint merge(
      ErplyResponsiveBreakpoint responsiveBreakpoint) {
  ...
  @override
  String toString() =>
      'ErplyResponsiveBreakpoint(breakpoint: $breakpoint, name: $name, behavior: $behavior, scaleFactor: $scaleFactor)';

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ErplyResponsiveBreakpoint &&
...
@immutable
class ErplyResponsiveBreakpointSegment {
  final double breakpoint;
  final ErplyResponsiveBreakpointBehavior segmentType;
  final ErplyResponsiveBreakpoint responsiveBreakpoint;

  @visibleForTesting
  const ErplyResponsiveBreakpointSegment(
      {required this.breakpoint,
      required this.segmentType,
      required this.responsiveBreakpoint});

  get isResize => segmentType == ErplyResponsiveBreakpointBehavior.RESIZE;

  get isAutoScale => segmentType == ErplyResponsiveBreakpointBehavior.AUTOSCALE;

  get isAutoScaleDown =>
      segmentType == ErplyResponsiveBreakpointBehavior.AUTOSCALEDOWN;

  get isTag => segmentType == ErplyResponsiveBreakpointBehavior.TAG;

  ErplyResponsiveBreakpointSegment copyWith({
    double? breakpoint,
    ErplyResponsiveBreakpointBehavior? segmentType,
    ErplyResponsiveBreakpoint? responsiveBreakpoint,
  }) =>
      ErplyResponsiveBreakpointSegment(
        breakpoint: breakpoint ?? this.breakpoint,
  ...
  ErplyResponsiveBreakpointSegment merge(
      ErplyResponsiveBreakpointSegment responsiveBreakpointSegment) {
    // Tag does not overwrite existing behavior.
    // Preserve existing values when merging.
  ...
  @override
  String toString() =>
      'ErplyResponsiveBreakpointSegment(breakpoint: $breakpoint, segmentType: $segmentType, responsiveBreakpoint: $responsiveBreakpoint)';

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is ErplyResponsiveBreakpointSegment &&
          runtimeType == other.runtimeType &&
          breakpoint == other.breakpoint &&
...
/// efficiently.
List<ErplyResponsiveBreakpointSegment> getBreakpointSegments(
    List<ErplyResponsiveBreakpoint> breakpoints,
    ErplyResponsiveBreakpoint defaultBreakpoint) {
  // Mutable breakpoints holder.
  List<ErplyResponsiveBreakpoint> breakpointsHolder = [];
  ...
  if (breakpointsHolder.isEmpty) {
    breakpointSegments.add(ErplyResponsiveBreakpointSegment(
        breakpoint: 0,
  ...
  List<ErplyResponsiveBreakpoint> breakpointTags =
  ...
  ErplyResponsiveBreakpoint? minWidthOverride = breakpointsHolder
  ...
  breakpointsHolder.sort(ErplyResponsiveUtils.breakpointComparator);
  breakpointTags.sort(ErplyResponsiveUtils.breakpointComparator);

  // Find the first breakpoint behavior.
  ErplyResponsiveBreakpoint initialBreakpoint = breakpointsHolder.first;

  // Breakpoint variable that holds the active behavior.
  // Used to calculate and remember the breakpoint behavior.
  ErplyResponsiveBreakpoint breakpointHolder = ErplyResponsiveBreakpoint(
  ...
  for (int i = 0; i < breakpointsHolder.length; i++) {
    ...
    ErplyResponsiveBreakpoint breakpoint = breakpointsHolder[i];
    // Segment calculation holder.
    ErplyResponsiveBreakpointSegment? breakpointSegmentHolder;
    switch (breakpoint.behavior) {
      case ErplyResponsiveBreakpointBehavior.RESIZE:
      case ErplyResponsiveBreakpointBehavior.AUTOSCALE:
        breakpointSegmentHolder = ErplyResponsiveBreakpointSegment(
            breakpoint: breakpoint.breakpoint,
            segmentType: breakpoint.behavior,
            responsiveBreakpoint: breakpoint);
        break;
      case ErplyResponsiveBreakpointBehavior.AUTOSCALEDOWN:
        ...
        ErplyResponsiveBreakpointSegment overrideBreakpointSegment =
        ...
        breakpointSegmentHolder = ErplyResponsiveBreakpointSegment(
            breakpoint: breakpoint.breakpoint,
            segmentType: breakpoint.behavior,
            responsiveBreakpoint: ErplyResponsiveBreakpoint(
                breakpoint: breakpoint.breakpoint,
                name: breakpoint.name,
                behavior: ErplyResponsiveBreakpointBehavior.AUTOSCALE,
                scaleFactor: breakpoint.scaleFactor));
        break;
      case ErplyResponsiveBreakpointBehavior.TAG:
        break;
    }
  ...
  for (int i = 0; i < breakpointTags.length; i++) {
    // Convenience variable.
    ErplyResponsiveBreakpoint breakpointTag = breakpointTags[i];
    ErplyResponsiveBreakpointSegment breakpointSegmentHolder =
    ...
    if (breakpointSegmentHolder.breakpoint == breakpointTag.breakpoint) {
      breakpointSegments[breakpointHolderIndex] =
          ErplyResponsiveBreakpointSegment(

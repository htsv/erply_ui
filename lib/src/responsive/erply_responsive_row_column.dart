...
enum ErplyResponsiveRowColumnType {
  ROW,
  COLUMN,
}
...
///
/// ErplyResponsiveRowColumn combines responsiveness
/// behaviors for managing rows and columns into one
/// convenience widget. This widget requires all [children]
/// to be [ErplyResponsiveRowColumnItem] widgets.
/// Row vs column layout is controlled by passing a
/// [ErplyResponsiveRowColumnType] to [layout].
/// Add spacing between widgets with [rowSpacing] and
/// [columnSpacing]. Add padding around widgets with
/// [rowPadding] and [columnPadding].
///
/// See [ErplyResponsiveRowColumnItem] for [Flex] and
/// [FlexFit] options.
class ErplyResponsiveRowColumn extends StatelessWidget {
  final List<ErplyResponsiveRowColumnItem> children;
  final ErplyResponsiveRowColumnType layout;
  ...
  get isRow => layout == ErplyResponsiveRowColumnType.ROW;
  get isColumn => layout == ErplyResponsiveRowColumnType.COLUMN;

  const ErplyResponsiveRowColumn(
      {super.key,
      this.children = const [],
  ...
  Widget build(BuildContext context) {
    if (layout == ErplyResponsiveRowColumnType.ROW) {
      return Padding(
        padding: rowPadding,
        child: Row(
  ...
  List<Widget> buildChildren(List<ErplyResponsiveRowColumnItem> children,
      bool rowColumn, double? spacing) {
    // Sort ErplyResponsiveRowColumnItems by their order.
    List<ErplyResponsiveRowColumnItem> childrenHolder = [];
...
/// A wrapper for [ErplyResponsiveRowColumn] children with
/// responsiveness.
///
/// Control the order of widgets within [ErplyResponsiveRowColumn]
...
class ErplyResponsiveRowColumnItem extends StatelessWidget {
  final Widget child;
  final int rowOrder;
  ...
  const ErplyResponsiveRowColumnItem(
      {super.key,
      required this.child,
  ...
  ErplyResponsiveRowColumnItem copyWith({
    int? rowOrder,
    int? columnOrder,
    bool? rowColumn,
    int? rowFlex,
